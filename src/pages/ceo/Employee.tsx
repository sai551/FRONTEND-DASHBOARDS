...
//   const [totalEmployeesDB, setTotalEmployeesDB] = useState(0); // total employees in DB
...
//         axios.get(`http://localhost:3000/employees/getAll?page=${pageNumber}`, getAuthHeaders()),
...
//       setPage(pageNumber);
//       setTotalEmployeesDB(empRes.data.pagination?.totalItems || empData.length); // total employees in DB
//     } catch (err) {
...
//   const activeEmployees = safeEmployees.filter(e => e?.status?.toLowerCase() === "active").length;
//   const inactiveEmployees = safeEmployees.filter(e => e?.status?.toLowerCase() === "inactive").length;
...
//             <div>
//               <h3 className="text-xs font-medium text-gray-500 uppercase">Total Employees (DB)</h3>
//               <p className="mt-1 text-2xl font-bold">{totalEmployeesDB}</p>
//             </div>
...
//               <p className="mt-1 text-2xl font-bold text-red-600">{inactiveEmployees}</p>
//             </div>
...
//       <Card>
//         <CardHeader>
//           <CardTitle>Total Employees (Current Page: {page})</CardTitle>
...
//                       <TableCell className="font-mono text-sm text-gray-600">{employee.employeeId}</TableCell>
//                       <TableCell>
//                         <div className="flex flex-col">
//                           <p className="font-semibold text-gray-900">{employee.firstName} {employee.lastName}</p>
//                           <p className="text-sm text-gray-500">{employee.username} - {employee.email}</p>
//                         </div>
//                       </TableCell>
//                       <TableCell>{employee.phone}</TableCell>
//                       <TableCell>{getRoleName(employee.roleId)}</TableCell>
//                       <TableCell><Badge variant="secondary">{getDepartmentName(employee.departmentId)}</Badge></TableCell>
//                       <TableCell>
//                         <Badge variant={employee.status?.toLowerCase() === "active" ? "default" : "secondary"}>
//                           {employee.status}
//                         </Badge>
//                       </TableCell>
...
//                             <DropdownMenuItem onClick={() => toast({ title: "View", description: employee.email })}>
//                               <Eye className="mr-2 h-4 w-4" /> View
//                             </DropdownMenuItem>
//                             <DropdownMenuItem onClick={() => toast({ title: "Edit Clicked", description: employee.firstName })}>
...
//                 ) : (
//                   <TableRow>
//                     <TableCell colSpan={8} className="text-center py-6 text-gray-500">No employees found</TableCell>
//                   </TableRow>
//                 )}
//               </TableBody>
//             </Table>
...
const Employee = () => {
  ...
  const fetchData = async (pageNumber = 1) => {
    ...
    try {
      ...
      // Total employees in DB comes from backend pagination totalItems
      setTotalEmployeesDB(empRes.data.pagination?.totalItems || empData.length);
    } catch (err) {
      console.error("Error fetching data:", err);
      toast({ title: "Error", description: "Failed to load employees" });
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Total Employees: {totalEmployeesDB}</CardTitle>
          <CardDescription>Manage your team members</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="rounded-md border overflow-x-auto">
                      ...
                      <TableCell className="text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem
                              onClick={() =>
                                toast({ title: "View", description: employee.email })
                              }
                            >
                              <Eye className="mr-2 h-4 w-4" /> View
                            </DropdownMenuItem>
                            <DropdownMenuItem
                              onClick={() =>
                                toast({ title: "Edit Clicked", description: employee.firstName })
                              }
                            >
                              <Edit className="mr-2 h-4 w-4" /> Edit
                            </DropdownMenuItem>
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent
          className="bg-white max-w-4xl my-1 max-h-[100vh] overflow-y-auto"
          id="scrollbar-hide"
        >
          <DialogHeader />
          <RegisterEmployee />
